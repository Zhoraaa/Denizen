...
margo_world:
    ...
    events:
        ...
        on player right clicks bedrock location_flagged:margo_core with:redcon:
            ...
            - define text 'При помощи Редкона, вы смогли восстановить Маргариту,|словно она никогда и не знала тех бед, что пришлись на её век.'

            - if <player.has_flag[team].advanced_matches[goners]>:
                - define text:->:<element['|Разобравшись в управлении, не без помощи старой команды. Вы смогли найти дорогу домой.|Опасная случайность, сподвигшая вас биться насмерть за право вернуться не пройдёт бесследно.|Но новые тяготы не так страшны, когда уже прошел через ад.|Впрочем, это уже совсем другая история...]>
            - if <player.has_flag[team].advanced_matches[rewrit]>:
                - define text:->:<element['|Укрывшись на этом дивном островке безопасности, вы смогли пережить перестройку этого мира в нечто новое, более совершенное.|В новообретённом мире будет место каждому.|Для признания ошибок,|для роста над собой,|для восстановления некогда утраченной дружбы.|Второй шанс, заслуженный кровью и потом.|Впрочем, это уже совсем другая история...]>

            - define text:->:<element[|Спасибо, что остаётесь с нами.|Следующая остановка - ССТ:Technical Invasion!|Конец ССТ: Lost memories.|Конец деменции.]>
